...
function Presenter() {
  ...
  /* Updates progress with the given test, which should have its results in it as well. */
    ...
    this.finished = function(elapsed) {
        // log all tests
        for (var ss in globalSection.subsections) {
            var subsection = globalSection.subsections[ss];
            if (subsection.totalTests > 0) {
                console.log(subsection.id+": "+subsection.totalPassed+" / "+subsection.totalTests+" = "+(subsection.totalPassed / subsection.totalTests * 100).toFixed(2)+"%");
            }
        }
